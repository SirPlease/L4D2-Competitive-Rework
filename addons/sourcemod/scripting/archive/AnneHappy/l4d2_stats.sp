	...
	if (GetClientTeam(attacker) == TEAM_SURVIVOR && GetClientTeam(victim) == TEAM_INFECTED) {
		...
		if (zombieclass == L4D2Infected_Boomer) {
			...
			if (strcmp(weapon, "weapon_melee") == 0) {
				...
				CPrintToChatAll("{green}★ {olive}%N {default}被 {olive}%N {default}用{blue}近战空爆", victim, attacker);
				...
				CPrintToChatAll("{green}★ {olive}%N {default}被 {olive}%N {blue}爆头", victim, attacker);
				...
				CPrintToChatAll("{green}★ {olive}%N {blue}%d枪 {default}空爆了 {olive}%N", \
