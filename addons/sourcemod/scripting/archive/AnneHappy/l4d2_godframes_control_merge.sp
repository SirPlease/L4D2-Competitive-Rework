...
public void OnPluginStart()
{
	...
	g_hRock = CreateConVar("gfc_rock_override", "1", "Allow hittables to always ignore godframes.", _, true, 0.0, true, 1.0);
	g_hWitch = CreateConVar("gfc_witch_override", "1", "Allow witches to always ignore godframes.", _, true, 0.0, true, 1.0);
	g_hFF = CreateConVar("gfc_ff_min_time", "0.8", "Minimum time before FF damage is allowed.", _, true, 0.0, true, 3.0);
	g_hSpit = CreateConVar("gfc_spit_extra_time", "0.0", "Additional godframe time before spit damage is allowed.", 0, true, 0.0, true, 3.0);
	...
	g_hHunter = CreateConVar("gfc_hunter_duration", "1.5", "How long should godframes after a pounce last?", _, true, 0.0, true, 3.0);
	...
	g_hCharger = CreateConVar("gfc_charger_duration", "1.8", "How long should godframes after a pummel last?", _, true, 0.0, true, 3.0);
	g_hSpitFlags = CreateConVar("gfc_spit_zc_flags", "15", "Which classes will be affected by extra spit protection time. 1 - Hunter. 2 - Smoker. 4 - Jockey. 8 - Charger.", _, true, 0.0, true, 15.0);
	g_hCommonFlags= CreateConVar("gfc_common_zc_flags", "15", "Which classes will be affected by extra common protection time. 1 - Hunter. 2 - Smoker. 4 - Jockey. 8 - Charger.", _, true, 0.0, true, 15.0);
