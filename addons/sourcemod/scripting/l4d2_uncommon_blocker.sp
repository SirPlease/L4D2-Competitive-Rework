...
int
	g_iBlockUncInfFlags = 0;

bool
	g_bIsPluginEnable = false;
...
public void OnPluginStart()
{
	...
	g_hPluginEnabled.AddChangeHook(Cvars_Changed);
	g_hBlockUncInfFlags.AddChangeHook(Cvars_Changed);
...
void CvarsToType()
{
	g_bIsPluginEnable = g_hPluginEnabled.BoolValue;
	g_iBlockUncInfFlags = g_hBlockUncInfFlags.IntValue;
}

public void OnEntityCreated(int iEntity, const char[] sClassName)
{
	if (sClassName[0] != 'i' || !g_bIsPluginEnable) {
		return;
	}
...
public void Hook_OnEntitySpawned(int iEntity)
{
	...
	PrintToChatAll("Uncommon infected spawned! Entity: %d, model: %s, gender: %d, plugin flag: %d, blocked: %s.", \
						iEntity, sModel, GetGender(iEntity), (1 << iUncommonInfected), (IsUncommonInfectedBlocked(iUncommonInfected)) ? "true" : "false");
#endif
...
#if DEBUG
	PrintToChatAll("2 Blocked uncommon infected! Entity: %d, location: %.0f %.0f %.0f.", iEntity, fLocation[0], fLocation[1], fLocation[2]);
#endif

	KillEntity(iEntity);													// kill the uncommon infected
	
	// some zombies have zero coordinates, temporary fix :D
...
{
	return (((1 << iUncommonInfected) & g_iBlockUncInfFlags) != 0);
}
