...
#if SM2_DEBUG
	PrintToChatAll("%t", "L4D2HybridScoremod_MapHealthBonusTempHealth", fMapBonus, fMapTempHealthBonus, fPermHpWorth, fTempHpWorth, iPillWorth);
#endif
}
...
Action:CmdBonus(client, args)
{
	...
	if (StrEqual(sCmdType, "full"))
	{
		...
		{
			PrintToChat(client, "%t", "L4D2HybridScoremod_1Bonus", PLUGIN_TAG, RoundToFloor(fSurvivorBonus[0]), RoundToFloor(fMapBonus + fMaxPillsBonus), CalculateBonusPercent(fSurvivorBonus[0]), sSurvivorState[0]);
		}
		PrintToChat(client, "%t", "L4D2HybridScoremod_BonusHBDBPills", PLUGIN_TAG, InSecondHalfOfRound() + 1, RoundToFloor(fHealthBonus + fDamageBonus + fPillsBonus), CalculateBonusPercent(fHealthBonus + fDamageBonus + fPillsBonus, fMapHealthBonus + fMapDamageBonus + fMaxPillsBonus), RoundToFloor(fHealthBonus), CalculateBonusPercent(fHealthBonus, fMapHealthBonus), RoundToFloor(fDamageBonus), CalculateBonusPercent(fDamageBonus, fMapDamageBonus), RoundToFloor(fPillsBonus), CalculateBonusPercent(fPillsBonus, fMaxPillsBonus));
		// R#1 Bonus: 556 <69.5%> [HB: 439 <73.1%> | DB: 117 <58.5%> | Pills: 90 <75.0%>]
	}
	else if (StrEqual(sCmdType, "lite"))
	{
		PrintToChat(client, "%t", "L4D2HybridScoremod_Bonus", PLUGIN_TAG, InSecondHalfOfRound() + 1, RoundToFloor(fHealthBonus + fDamageBonus + fPillsBonus), CalculateBonusPercent(fHealthBonus + fDamageBonus + fPillsBonus, fMapHealthBonus + fMapDamageBonus + fMaxPillsBonus));
		// R#1 Bonus: 556 <69.5%>
	}
	else
	{
...
Action:CmdMapInfo(client, args)
{
	...
	PrintToChat(client, "%t", "L4D2HybridScoremod_HybridBonusMapInfo", iTeamSize, iTeamSize);
	PrintToChat(client, "%t", "L4D2HybridScoremod_Distance", iMapDistance);
	PrintToChat(client, "%t", "L4D2HybridScoremod_TotalBonus1000", RoundToFloor(fTotalBonus));
	PrintToChat(client, "%t", "L4D2HybridScoremod_HealthBonus", RoundToFloor(fMapHealthBonus), CalculateBonusPercent(fMapHealthBonus, fTotalBonus));
	PrintToChat(client, "%t", "L4D2HybridScoremod_DamageBonus", RoundToFloor(fMapDamageBonus), CalculateBonusPercent(fMapDamageBonus, fTotalBonus));
	PrintToChat(client, "%t", "L4D2HybridScoremod_PillsBonusMax", iPillWorth, RoundToFloor(fMaxPillsBonus), CalculateBonusPercent(fMaxPillsBonus, fTotalBonus));
	PrintToChat(client, "%t", "L4D2HybridScoremod_Tiebreaker", iPillWorth);
...
#if SM2_DEBUG
	if (GetSurvivorTemporaryHealth(victim) > 0) PrintToChatAll("%t", "L4D2HybridScoremod_TempHPDamage", victim, GetSurvivorTemporaryHealth(victim), damage);
#endif
...
#if SM2_DEBUG
	PrintToChatAll("%t", "L4D2HybridScoremod_LostTempHPAfterAttacked", victim, iTempHealth[victim] - (IsPlayerAlive(victim) ? GetSurvivorTemporaryHealth(victim) : 0), damage);
#endif
...
{
#if SM2_DEBUG
	PrintToChatAll("%t", "L4D2HybridScoremod_DirectorEndVersusModeRound", InSecondHalfOfRound(), countSurvivors);
#endif
	if (bRoundOver)
		return Plugin_Continue;
	...
	#if SM2_DEBUG
		PrintToChatAll("%t", "L4D2HybridScoremod_SurvivalBonusCvarUpdatedValue", GetConVarInt(hCvarValveSurvivalBonus), iSurvivalMultiplier);
	#endif
	}
	else
	{
...
Action:PrintRoundEndStats(Handle:timer) 
{
	...
	{
		PrintToChatAll("%t", "L4D2HybridScoremod_RoundBonus", PLUGIN_TAG, (i + 1), RoundToFloor(fSurvivorBonus[i]), RoundToFloor(fMapBonus + float(iPillWorth * iTeamSize)), CalculateBonusPercent(fSurvivorBonus[i]), sSurvivorState[i]);
		// [EQSM :: Round 1] Bonus: 487/1200 <42.7%> [3/4]
	}
	
	if (InSecondHalfOfRound() && bTiebreakerEligibility[0] && bTiebreakerEligibility[1])
	{
		PrintToChatAll("%t", "L4D2HybridScoremod_TiebreakerTeam1Team2", PLUGIN_TAG, iSiDamage[0], iSiDamage[1]);
		if (iSiDamage[0] == iSiDamage[1])
		{
			PrintToChatAll("%t", "L4D2HybridScoremod_TeamsPerformedAbsolutelyEqualImpossible", PLUGIN_TAG);
		}
	}
...
Float:GetSurvivorHealthBonus()
{
	...
	for (new i = 1; i <= MaxClients && survivorCount < iTeamSize; i++)
	{
		if (IsSurvivor(i))
		{
			...
			#if SM2_DEBUG
				PrintToChatAll("%t", "L4D2HybridScoremod_AddingPermHPBonusContribution", i, GetSurvivorPermanentHealth(i), GetSurvivorPermanentHealth(i) * fPermHpWorth, fHealthBonus);
			#endif
			}
		}
	}
...
#if SM2_DEBUG
	PrintToChatAll("%t", "L4D2HybridScoremod_AddingTempHPBonusEligible", fDamageBonus, survivalMultiplier);
#endif
...
{			
	...
	for (new i = 1; i <= MaxClients && survivorCount < iTeamSize; i++)
	{
		if (IsSurvivor(i))
		{
			...
			#if SM2_DEBUG
				PrintToChatAll("%t", "L4D2HybridScoremod_AddingPillsContributionTotalBonus", i, pillsBonus);
			#endif
			}
		}
	}
