...
public int ChampVoteHandler(Menu menu, MenuAction action, int param1, int param2) {
	if (action == MenuAction_Display) {
		...
		if (percent < limit) {
			...
			PrintToChatAll("\x04[提示]\x05%t  \x01[%d,%d,%d]", "Vote Failed", RoundToNearest(100.0*limit), RoundToNearest(100.0*percent), playerCount, yesVotes, noVotes, abstention);
		} else {
			PrintToChatAll("\x04[提示]\x05%t  \x01[%d,%d,%d]", "Vote Successful", RoundToNearest(100.0*percent), playerCount, yesVotes, noVotes, abstention);
			...
			if (mapIndex < 0) {
				...
				for (int client = 1; client <= MaxClients; client++) {
					if (IsClientInGame(client)) {
						...
						PrintToChat(client,"\x04[提示]\x01%t\x01", "Mission is now winning the vote", colorizedName);
					}
				}			
			} else {
				...
				for (int client = 1; client <= MaxClients; client++) {
					if (IsClientInGame(client)) {
						LMM_GetMapLocalizedName(g_iGameMode, missionIndex, mapIndex, localizedName, sizeof(localizedName), client);
						Format(colorizedName, sizeof(colorizedName), "\x04%s\x01", localizedName);
						PrintToChat(client,"\x04[提示]\x01%t", "Map is now winning the vote", colorizedName);
					}
				}
			}
			
			Format(colorizedName, sizeof(colorizedName), "\x04%s\x01", mapName);
			PrintToChatAll("\x04[提示]\x01%t", "Changing map", colorizedName);
...
public void OnPluginStart() {
	...
	CreateConVar("acs_version", PLUGIN_VERSION, "(ACS)自动换图插件的版本.", FCVAR_SPONLY|FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_DONTRECORD);
	g_hCVar_VotingEnabled = CreateConVar("acs_voting_system_enabled", "1", "启用玩家投票换图? 0=禁用, 1=启用.", _, true, 0.0, true, 1.0);
	g_hCVar_VoteWinnerSoundEnabled = CreateConVar("acs_voting_sound_enabled", "1", "打开投票换图菜单时播放声音? 0=禁用, 1=启用.", _, true, 0.0, true, 1.0);
	g_hCVar_VotingAdMode = CreateConVar("acs_voting_ad_mode", "3", "设置在地图开始处宣传投票的方式. 0=禁用, 1=屏幕中下, 2=聊天窗, 3=打开投票菜单. \n *注意:这仅在救援或清道夫显示一次*", _, true, 0.0, true, 3.0);
	g_hCVar_VotingAdDelayTime = CreateConVar("acs_voting_ad_delay_time", "10.0", "幸存者离开开始区域后延迟多少秒显示下一张地图的更换方式. \n *注意：如果服务器已启动，则在.cfg文件中进行更改会在更改之前进行两次映射更改*", _, true, 0.1, false);
	g_hCVar_NextMapMenuOptions = CreateConVar("acs_next_map_menu_options", "0", "设置投票菜单中显示的地图类型. 0=官方和附加地图(默认), 1=仅限官方图, 2=当前地图的类型.");
	g_hCVar_NextMapMenuExcludes = CreateConVar("acs_next_map_menu_excludes", "0", "在投票菜单中排除当前地图? 0=不排除(默认), 1=排除当前地图.");
	g_hCVar_NextMapMenuOrder= CreateConVar("acs_next_map_menu_order", "0", "设置投票菜单中地图的显示顺序. 0=官方图,然后是附加地图(默认), 1=随机.");
	g_hCVar_NextMapAdMode = CreateConVar("acs_next_map_ad_mode", "2", "设置救援关公告下一张地图信息的方式. 0=禁用, 1=屏幕中下, 2=聊天窗.", _, true, 0.0, true, 2.0);
	g_hCVar_NextMapAdInterval = CreateConVar("acs_next_map_ad_interval", "60.0", "救援关公告下一张地图信息的时间(秒).", _, true, 60.0, false);
	g_hCVar_MaxFinaleFailures = CreateConVar("acs_max_coop_finale_failures", "0", "设置战役模式救援关幸存者团灭多少次后自动换图? 0=禁用.", _, true, 0.0, false);
	g_hCVar_ChMapPolicy =  CreateConVar("acs_chmap_policy", "2", " 启用指令 !chmap 和 !chmap2 投票换图?\n 0=禁用(默认).\n 1=启用(弃权视为否定). \n 2=启用(弃权视为同意)");	
	g_hCVar_ChMapBroadcastInterval =  CreateConVar("acs_chmap_broadcast_interval", "180.0", "设置循环公告 \"!chmap\" 指令的时间(秒).");	
	g_hCVar_PreventEmptyServer =  CreateConVar("acs_prevent_empty_server", "1", "当服务器是附加战役且没有玩家在服务器时自动切换到第一个可用的官方图. 0=禁用, 1=启用.", _, true, 0.0, true, 1.0);	
...
public Action Command_ChangeMapVote(int iClient, int args) {
	if (g_hCVar_ChMapPolicy.IntValue < 1) {
		PrintToChat(iClient, "\x04[提示]\x05管理员没有开启\x03!chmap\x05投票换图指令.");
...
public Action Command_ChangeMapVote2(int iClient, int args) {
	if (g_hCVar_ChMapPolicy.IntValue < 1) {
		PrintToChat(iClient, "\x04[提示]\x05管理员没有开启\x03!chmap2\x05投票换图指令.");
		return Plugin_Handled;	// Disabled
	}
...
public Action Timer_WelcomeMessage(Handle timer, any param) {
	PrintToChatAll("\x04[提示]\x01 %t", "Change map advertise", "\x04!chmap\x01");
}
		...
		if(StrEqual(strCurrentMap, mapName, false)) {
			for (int client = 1; client <= MaxClients; client++) {
				if (IsClientInGame(client)) {
					...
					PrintToChat(client, "\x04[提示]\x05 %t", "Campaign finished", colorizedname);
				}
			}
				...
				if(IsMapValid(mapName)) {
					for (int client = 1; client <= MaxClients; client++) {
						if (IsClientInGame(client)) {
							...
							Format(colorizedname, sizeof(colorizedname), "\x04%s\x05", localizedName);
							PrintToChat(client, "\x04[提示]\x05 %t", "Switching to the vote winner", colorizedname);
						}
					}
			...
			if(IsMapValid(mapName)) {
				for (int client = 1; client <= MaxClients; client++) {
					if (IsClientInGame(client)) {
						ACS_GetLocalizedMissionName(g_iGameMode, cycleIndex, client, localizedName, sizeof(localizedName));
						Format(colorizedname, sizeof(colorizedname), "\x04%s\x05", localizedName);
						PrintToChat(client, "\x04[提示]\x05 %t", "Switching campaign to", colorizedname);
					}
				}
				...
				if(IsMapValid(mapName)) {
					for (int client = 1; client <= MaxClients; client++) {
						if (IsClientInGame(client)) {
							LMM_GetMapLocalizedName(g_iGameMode, missionIndex, mapIndex, localizedName, sizeof(localizedName), client);
							Format(colorizedname, sizeof(colorizedname), "\x04%s\x05", localizedName);
							PrintToChat(client, "\x04[提示]\x05 %t", "Switching map to", colorizedname);
						}
					}	
...
void DisplayNextMapTo_Scavenge(int client, bool replyCMD) {
	...
	if(g_iWinningMapVotes > 0) {
		...
		if (replyCMD) {
			ReplyToCommand(client, "\x04[提示]\x05 %t",
			...
			PrintToChat(client, "\x04[提示]\x05 %t",
				g_iWinningMapIndices_Len == 1 ?
					"The next map is currently" :
					"The next map will be one of",
				strbuf);
...
void DisplayNextMapTo(int client, bool replyCMD) {
	...
	if (g_iWinningMapVotes > 0) {
		...
		if (replyCMD) {
			Format(colorizedName, sizeof(colorizedName), "\x04%s\x05", localizedName);
			ReplyToCommand(client, "\x04[提示]\x05 %t", "The next campaign is currently", colorizedName);
		} else if (g_hCVar_NextMapAdMode.IntValue == DISPLAY_MODE_HINT) {
			PrintHintText(client, "%t", "The next campaign is currently", localizedName);
		} else if (g_hCVar_NextMapAdMode.IntValue == DISPLAY_MODE_CHAT) {
			Format(colorizedName, sizeof(colorizedName), "\x04%s\x05", localizedName);
			PrintToChat(client, "\x04[提示]\x05 %t", "The next campaign is currently", colorizedName);
		}
	}
}
...
public Action MapVote(int iClient, int args) {
	if(!g_hCVar_VotingEnabled.BoolValue) {
		ReplyToCommand(iClient, "\x04[提示]\x01 %t", "Voting is disable");
		return;
	}
	
	if(!OnFinaleOrScavengeMap()) {
		PrintToChat(iClient, "\x04[提示]\x01 %t", "Voting is not available");
		return;
	}
...
public Action DisplayCurrentVotes(int iClient, int args) {
	...
	if(hasVoted()) {
		...
		ReplyToCommand(iClient, "\x04[提示]\x01 %t", "No one has voted yet");	
	}
	...
	for(int iClient = 1;iClient <= MaxClients; iClient++) {
		if(
			...
			switch(g_hCVar_VotingAdMode.IntValue) {
				...
				case DISPLAY_MODE_CHAT: PrintToChat(iClient, "\x04[提示]\x05 %t", "Map vote advertise", "\x04!mapvote\x05", "\x04!mapvotes\x05");
			}
